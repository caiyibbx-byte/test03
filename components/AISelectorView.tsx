...
import { Bot, MessageSquare, Send, Settings, Sparkles, Filter, ChevronRight } from 'lucide-react';

const AISelectorView: React.FC = () => {
      ...
      <div className="w-96 flex flex-col space-y-4">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="space-y-3">
            {[
              { title: '特高压自动化系统扩建', match: 95, tag: '极高匹配' },
              { title: '苏通大桥跨江塔基维护', match: 82, tag: '高匹配' },
            ].map((item, i) => (
              <div key={i} className="group p-3 border border-slate-100 rounded-lg hover:border-blue-200 hover:bg-blue-50 transition-all cursor-pointer">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex-1">
          ...
          <button className="w-full mt-4 py-2 border border-slate-200 rounded-lg text-sm text-slate-600 hover:bg-slate-50">
            编辑高级规则
          </button>
        </div>
      </div>
